...
Inspected the whole repo first; the **committed tree (119 tracked files) was already clean** — no stale/duplicate tracked files. All removals were untracked/generated artifacts or out-of-repo scratch, so `git status` shows only the `.gitignore` change.

### Removed
- `app/dist/` (~5.1 MB) — Vite build output. Untracked + gitignored (`.gitignore:2 dist`); Vercel rebuilds fresh (`outputDirectory: dist`, `buildCommand: npm run build`); no source refs. Reproducible via `npm run build` (verified — it regenerated).
- `app/tsconfig.tsbuildinfo`, `app/tsconfig.node.tsbuildinfo` (~16 KB) — TS incremental cache. Untracked + gitignored (`*.tsbuildinfo`); regenerated by `tsc`.
- `.playwright-mcp/` artifacts (148 files, ~659 KB: `console-*.log`, `page-*.yml`, screenshots) — generated Playwright MCP scratch, outside the git repo, not config. Directory kept; MCP recreates files as needed.
- Stray root `package-lock.json` (89 B, `"packages": {}`) — orphaned empty lockfile in the parent folder (the real one is `app/package-lock.json`, 109 KB). No root node project references it.

### Kept intentionally
- All 119 tracked files (src/, configs, `package.json`/lockfile, `index.html`, `README.md`, `QA_REPORT.md`, `AGENTS_SKILLS_EXPLAINED.md`, `.env.example`, `vercel.json`, tailwind/postcss/tsconfig/vite).
- `public/` — `cg.png` (favicon in `index.html`) and `logo-vsk.png` (used by 4 src files).
- `scripts/` — `generateSeed.py`/`attachGsqac.mjs`/`verify.mjs` are wired into npm scripts; the rest are dev/QA tools.
- `.agents/` + `skills-lock.json` (gitignored skills tooling), root `.claude/` (workflow config), `Docs/` (reference data/design docs).

### Unsure / future review
- `public/favicon.png` — tracked but not referenced anywhere (the page icon is `cg.png`). Kept (conventional, tiny public asset; removing a tracked asset for ~negligible gain is riskier than keeping).
- `Docs/logout-svgrepo-com.svg` — referenced only in an `Icon.tsx` comment; the file is already absent and the logout icon is inlined, so the app builds fine. No action.
- `.agents/skills/huashu-design/` (~32 MB media) — gitignored, but the largest folder on disk; optional future trim if local size matters (see `AGENTS_SKILLS_EXPLAINED.md`).

### .gitignore
- Added `*.log`, `coverage/`, `playwright-report/`, `test-results/`, `.playwright-mcp/` (none were previously covered; `dist`, `*.tsbuildinfo`, `.env*`, `.DS_Store`, `node_modules` already were).

### Validation
- `npm run build`: **pass** (regenerated `dist/` — confirms reproducibility).
- `npm run typecheck`: **pass**.
- `npm run lint`: stub (no eslint configured).
- `git status --short`: only ` M .gitignore` — no committed/source file changed.
